    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if attention_mask is not None:
            ...
            attention_mask = (
                torch.arange(hidden_states.shape[1], device=hidden_states.device) < output_lengths[:, None]
            )
            attention_mask = _prepare_4d_attention_mask(attention_mask, input_features.dtype)
